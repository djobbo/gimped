...
describe("ReplayCodec", () => {
  it("round-trips a minimal replay", async () => {
    const replay = minimal();
    const decoded = await run(encode(replay).pipe(Effect.flatMap(decode)));
    expect(decoded).toEqual(replay);
  });

  it("round-trips input rows that carry an input value", async () => {
    const replay: Replay = { ...minimal(), inputs: [{ entityId: 1, time: 16, input: 512 }] };
    const decoded = await run(encode(replay).pipe(Effect.flatMap(decode)));
    expect(decoded).toEqual(replay);
  });

  it("round-trips scores, events and a handicap", async () => {
    const base = minimal();
    const replay: Replay = {
      ...base,
  ...
  it("fails when the setup checksum does not match the players", async () => {
    const bytes = await run(encode(minimal()));
    const other = await run(encode(withColorScheme(minimal(), 7)));
    const result = await run(Effect.result(decode(corruptSetupChecksum(bytes, other))));
    expect(result._tag).toBe("Failure");
    if (result._tag === "Failure") expect(result.failure).toBeInstanceOf(ChecksumMismatch);
  });

  it("fails when the replay declares more than five hero slots", async () => {
    ...
    bits.writeU32(1);
    bits.writeU16(6);
    const result = await run(Effect.result(decode(bits.toUint8Array())));
    expect(result._tag).toBe("Failure");
  ...
  it("fails on a truncated bitstream", async () => {
    const bytes = await run(encode(minimal()));
    const result = await run(Effect.result(decode(bytes.subarray(0, 20))));
    expect(result._tag).toBe("Failure");
    if (result._tag === "Failure") {
      expect(result.failure).toBeInstanceOf(InvalidReplay);
      if (result.failure instanceof InvalidReplay) {
