...
layer(AppLive)("GithubRelease", (it) => {
  ...
  it.effect("emits StepProgress with fraction when Content-Length is present", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const github = yield* GithubRelease;
      ...
      const destDir = yield* fs.makeTempDirectory({ prefix: "gh-dest-" });
      yield* github
        .downloadLatestAsset("owner/tool", destDir, (name) => name === ASSET_NAME, "EnsureJpexs")
        .pipe(Effect.provide(reporterLayer));

      const recorded = yield* Ref.get(events);
      const progress = recorded.filter((event) => event._tag === "StepProgress");
      expect(progress.length).toBeGreaterThan(0);
      expect(progress.every((event) => event.step === "EnsureJpexs")).toBe(true);
