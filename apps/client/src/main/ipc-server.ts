...
// port on every window (re)load. It's a Context.Service because a layer publishes
// its values as services — index.ts reads `bind` back via runtime.runPromise(RpcPortHandoff).
export class RpcPortHandoff extends Context.Service<
  RpcPortHandoff,
  { readonly bind: (port: IpcServerPort) => void }
>()("gimped/client/RpcPortHandoff") {}
...
export const layerIpcServer: Layer.Layer<RpcServer.Protocol | RpcPortHandoff> = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const portInbox = yield* Queue.unbounded<IpcServerPort>();
    ...
    const protocol = Layer.effect(
      ...
      RpcServer.Protocol.make(
        Effect.fnUntraced(function* (writeRequest) {
          ...
          const inbound = yield* Queue.unbounded<readonly [number, FromClientEncoded]>();
          let nextClientId = 0;
            ...
            const handler = (event: { data: string | Uint8Array }): void => {
              try {
                for (const message of parser.decode(event.data)) {
                  // SAFETY: this server protocol only inbound-decodes FromClientEncoded frames.
